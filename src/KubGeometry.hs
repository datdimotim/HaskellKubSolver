...
ugolPerm np = fst . x1ElemRotate np . (, 0) 


x2MoveSlow :: Povorot -> Pos -> Pos
x2MoveSlow = moveInCoordsSlow toX2 fromX2 (mainRotatesToFullRotates ugolPerm)

rebroPerm :: Povorot -> Int -> Int
rebroPerm np = fst . rebroCubieRotate np . (, 0)
...
y2MoveSlow = let 
                ...
                proj   = take 8
             in 
                moveInCoordsSlow (toY2 . proj) (extend . fromY2) (mainRotatesToFullRotates rebroPerm)
...
z2MoveSlow = let 
             ...
             in 
                moveInCoordsSlow (toZ2 . proj) (extend . fromZ2) (mainRotatesToFullRotates rebroPerm)
...
y1MoveCubie np = map snd . sortOn fst . map (rebroCubieRotate np) . zip [0..]
